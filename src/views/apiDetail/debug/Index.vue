<template>
  <div>
    ...
    <Title :title="t('apiPage.title.responses')">
      ...
      <EventPopover :title="t('apiPage.debug.event.after')" :eventData="eventData.after" />
    </Title>
...
import { handleRequestBeforeEvent, handleRequestAfterEvent, ResultState } from "./handleEvent";
  ...
  setup(props) {
    ...
    function sendRequest(url: string, data?: any) {
      ...
      if (props.detail.before && props.detail.before.length) {
        const beforeEvent = handleRequestBeforeEvent(props.detail.before, json, store);
        json = beforeEvent.json;
        state.eventData.before = beforeEvent.result;
      }
      Axios(url, json)
        .then((res) => {
          // 执行后置方法
          if (props.detail.after && props.detail.after.length) {
            const afterEvent = handleRequestAfterEvent(props.detail.after, json, res, store);
            state.eventData.after = afterEvent as ResultState;
          }
          console.log(res);
          state.loading = false;
